...
public class OrderController {
    ...
    @PostMapping("/order")
    public ResponseEntity<?> addOrder(@RequestBody OrderDto input, @RequestHeader("username") String username) {

        Optional<Order> result = orderService.addOrder(input, username);
        return result.map(order -> ResponseEntity.status(HttpStatus.CREATED).body(order))
